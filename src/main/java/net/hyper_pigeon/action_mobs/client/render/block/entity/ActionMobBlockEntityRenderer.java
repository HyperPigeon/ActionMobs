...
public record ActionMobBlockEntityRenderer(
        BlockEntityRendererFactory.Context context) implements BlockEntityRenderer<ActionMobBlockEntity> {
    @Override
    public void render(ActionMobBlockEntity blockEntity, float tickProgress, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay, Vec3d cameraPos) {
        if (blockEntity.getWorld() == null || blockEntity.getWorld().getBlockState(blockEntity.getPos()).isAir())
            return;
